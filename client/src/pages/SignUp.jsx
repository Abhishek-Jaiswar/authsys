...
const SignUp = () => {
    ...
    const passwordRequirements = useMemo(
        () => [
            { test: 'At least 8 characters', met: password.length >= 8 },
            { test: 'One uppercase letter', met: /[A-Z]/.test(password) },
            { test: 'One lowercase letter', met: /[a-z]/.test(password) },
            { test: 'One number', met: /\d/.test(password) },
        ],
        [password]
    );
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            if (!fullname) {
                ...
                newErrors.fullname = validationRules.fullname.message;
                hasError = true;
            }
            ...
            const response = await api.post('/api/v1/user/register', { fullname, email, password });
            ...
            setErrors({
                ...errors,
                general: error.response?.data?.message || 'Failed to create your account, try again.',
            });
            toast.error(error.response?.data?.message || 'Failed to create your account, try again.');
        } finally {
            setIsLoading(false);
        }
    };
        ...
        <div className="min-h-screen flex justify-center items-center bg-gray-100">
            <div className="max-w-md w-full border border-neutral-200 rounded-md bg-white p-6">
                <h1 className="text-2xl font-bold text-neutral-700 mb-6">
                    Create your <span className="text-neutral-900">AuthSys</span> account
                </h1>
                <form onSubmit={handleSubmit} className="space-y-4">
                    ...
                    <div className="flex flex-col space-y-1">
                        ...
                        <div className="relative">
                            <FiUser className="absolute left-3 top-1/2 -translate-y-1/2 text-neutral-600 text-xl" />
                            <input
                                type="text"
                                id="fullname"
                                value={fullname}
                                ...
                                className="w-full border border-neutral-200 rounded-md outline-none pl-10 pr-3 py-2 text-sm focus:ring-2 focus:ring-blue-400 focus:ring-offset-1"
                            />
                        </div>
                        {errors.fullname && (
                    ...
                    <div className="flex flex-col space-y-1">
                        ...
                        {showPassReq && <div className={`text-sm text-neutral-600 mt-2 ${showPassReq && " transition-all duration-200 ease-in"}`}>
                    ...
                    <div className='text-center'>
                        <p className='text-sm text-neutral-800'>Allready have an account?<Link to='/login' className=' hover:text-red-500 underline active:text-blue-600'>{" "} Login here</Link></p>
